...
export async function GET(request: NextRequest) {
  ...
  const sites = await prisma.site.findMany({ where: scopedCompanyWhere(context.user), include: { client: true }, orderBy: { name: "asc" } });
  return apiOk({ sites });
}
...
export async function POST(request: NextRequest) {
  ...
  const client = await prisma.client.findFirst({ where: { id: parsed.data.clientId, ...scopedCompanyWhere(context.user) } });
  if (!client) return apiError("Client introuvable", 404);
  const site = await prisma.site.create({ data: { ...parsed.data, companyId: client.companyId } });
...
export async function PATCH(request: NextRequest) {
  ...
  if (!parsed.success) return apiError("Site invalide", 400, parsed.error.flatten());
  const current = await prisma.site.findFirst({ where: { id: parsed.data.id, ...scopedCompanyWhere(context.user) } });
